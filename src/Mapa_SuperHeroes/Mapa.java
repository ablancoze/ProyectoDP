...
/**Metodos privados de la clase mapa */
	...
	private void distribuirArmas(LinkedList<Sala> salas,Arma [] armasSalas) {
		...
		while(aux < armasSalas.length) {
			if (!salas.isEmpty()){
				for (int j = aux; salas.getFirst().tama駉Armas()<5 && armasSalas.length > j ; j++) {
	...
	private void obtenerTodosLosCaminos(Integer origen, int destino, LinkedList<Integer>visitados,LinkedList<Sala>recorrido,LinkedList<LinkedList<Sala>>todosLosCaminos){
